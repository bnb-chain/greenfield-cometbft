...
func applyBlock(t *testing.T, stateStore sm.Store, st sm.State, blk *types.Block, proxyApp proxy.AppConns) sm.State {
	...
	blockExec := sm.NewBlockExecutor(stateStore, log.TestingLogger(), proxyApp.Consensus(), nil, mempool, evpool)

	bps, err := blk.MakePartSet(testPartSize)
	require.NoError(t, err)
	blkID := types.BlockID{Hash: blk.Hash(), PartSetHeader: bps.Header()}
	newState, _, err := blockExec.ApplyBlock(st, blkID, blk, false, 1)
	require.NoError(t, err)
	return newState
}
...
func makeBlocks(n int, state sm.State, privVals []types.PrivValidator) ([]*types.Block, error) {
	...
	for i := 0; i < n; i++ {
		...
		block := state.MakeBlock(height, test.MakeNTxs(height, 10), lastCommit, nil, nil, state.LastValidators.Proposer.Address)
		blocks[i] = block
